...
def register_user():
    ...
    # 文字数制限のチェック
    if len(username) > 20:
        flash('ユーザー名は20文字以下にしてください')
        return redirect(url_for('register'))
    
    if len(password) < 6 or len(password) > 20:
        flash('パスワードは6文字以上20文字以下にしてください')
        return redirect(url_for('register'))
...
def tasks(user_id):
    ...
    tasks = Task.query.filter_by(user_id=user_id, completed=False).order_by(Task.due_date).all()
    completed_tasks = Task.query.filter_by(user_id=user_id, completed=True).order_by(Task.due_date.desc()).all()
    now = datetime.now()
